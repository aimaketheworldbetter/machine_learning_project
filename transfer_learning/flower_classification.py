...
def get_image_path(image_lists, images_dir, label_name, index, category):
...
def get_random_cached_bottlenecks(sess, n_classes, image_lists, how_many, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    for i in range(how_many):
        ...
        bottleneck = get_or_create_bottleneck(sess, image_lists,
                                              label_name, image_index, category,
...
def main(_):
    #读取所有的图片
    image_lists = create_image_list(testing_percentage=TEST_PERCENTAGE, validation_percentage=VALIDATION_PERCENTAGE)
    ...
    bottleneck_tensor, jpeg_data_tensor = tf.import_graph_def(graph_def, return_elements=[BOTTLENECK_TENSOR_NAME, JEPG_DATA_TENSOR_NAME])
    bottleneck_input = tf.placeholder(tf.float32, [None, BOTTLENECK_TENSOR_SIZE], name='BottleneckInputPlaceholder')
    ground_truth_input = tf.placeholder(tf.float32, [None, n_classes], name="GroundTruthInputPlaceholder")
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if i % 200 == 0 or i + 1 == STEPS:
                ...
                print("Step {} validation accuracy on random sampled {} examples = {}%".format(i, BATCH, validation_accuracy*100))
